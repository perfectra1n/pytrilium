...
def update_version_in_file(file_path, old_version, new_version, pattern):
    ...
    content = re.sub(pattern.format(old_version=re.escape(old_version)), 
...
def update_all_versions(old_version, new_version):
    ...
    if update_version_in_file("pyproject.toml", old_version, new_version, 
                              r'version = "{old_version}"'):
        files_updated.append("pyproject.toml")
    
    # Update User-Agent in PyTriliumClient.py
    if update_version_in_file("pytrilium/PyTriliumClient.py", old_version, new_version,
                              r'pytrilium/{old_version}'):
...
def main():
    ...
    parser.add_argument("--create-tag", action="store_true", 
                        help="Create a git tag after updating versions")
    parser.add_argument("--push-tag", action="store_true", 
                        help="Push the git tag to origin (implies --create-tag)")
    ...
    if not args.create_tag:
        print(f"   3. Create and push a git tag: git tag -a v{args.new_version} -m 'Release v{args.new_version}' && git push origin v{args.new_version}")
